...
function workerLabel(w){return w.number?`${w.number} - ${w.name}`:w.name}
function workerName(db,id){const w=db.workers.find(x=>x.id===id);return w?workerLabel(w):id}
function teamAutoName(db,machineId){const machine=db.machines.find(x=>x.id===machineId);if(!machine)return"";const count=db.teams.filter(t=>t.machineId===machineId).length+1;return `E${count} · ${machine.name}`}
